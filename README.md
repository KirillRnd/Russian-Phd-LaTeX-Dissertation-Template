# Диссертация А. Д. Корнеевой

Рабочая ветка содержит LaTeX-версию диссертации, перенесённую из канонических
DOCX `ОРИГИНАЛ/13.04 ДИССЕР/ВЕРСИЯ_3`. Техническая история, ограничения и
воспроизводимый процесс переноса описаны в [migration/README.md](migration/README.md).

Диссертация всегда собирается как два самостоятельных тома, которые затем
объединяются в один итоговый PDF:

* [dissertation-volume1.tex](dissertation-volume1.tex) — введение,
  историко-правовой контекст и глава 1;
* [dissertation-volume2.tex](dissertation-volume2.tex) — главы 2–3, заключение,
  структурированный список литературы и приложения;
* [dissertation.tex](dissertation.tex) — PDF-обёртка, которая всегда включает
  оба уже собранных тома;
* `make dissertation` — единственная основная команда сборки: она запускает
  `tools/build_dissertation.py`, пересобирает оба тома и создаёт
  `output/pdf/dissertation-korneeva.pdf`.

Основная цель не принимает параметр выбора тома и не может выпустить один том
вместо двух.
